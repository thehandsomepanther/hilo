...
function handleTurnCreds (request, env) {
  const headers = { ...corsHeaders, Vary: 'Origin', 'Content-Type': 'application/json' }

  if (!env.METERED_USERNAME || !env.METERED_CREDENTIAL) {
    return new Response(JSON.stringify([]), { status: 200, headers })
  }

  const servers = [
    { urls: 'stun:stun.relay.metered.ca:80' },
    { urls: 'turn:standard.relay.metered.ca:80',                 username: env.METERED_USERNAME, credential: env.METERED_CREDENTIAL },
    { urls: 'turn:standard.relay.metered.ca:80?transport=tcp',   username: env.METERED_USERNAME, credential: env.METERED_CREDENTIAL },
    { urls: 'turn:standard.relay.metered.ca:443',                username: env.METERED_USERNAME, credential: env.METERED_CREDENTIAL },
    { urls: 'turns:standard.relay.metered.ca:443?transport=tcp', username: env.METERED_USERNAME, credential: env.METERED_CREDENTIAL },
  ]

  return new Response(JSON.stringify(servers), { status: 200, headers })
}
...
export default {
  async fetch (request, env, context) {
    ...
    if (request.method === 'GET' && url.pathname === '/turn-creds') {
      return handleTurnCreds(request, env)
    }
